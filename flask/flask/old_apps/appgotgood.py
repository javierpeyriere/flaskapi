...
def characters():
    ...
    # Get character
    with db_conn.cursor() as cursor:
        cursor.execute(""" select p.character_id, p.name, p.father, p.mother, p.house,  d.gender, d.death_chapter, d.book_of_death, p.DateoFdeath,
                        CASE 
                        WHEN p.DateoFdeath is null
                        then 'Alive'
                        else 'Dead'
